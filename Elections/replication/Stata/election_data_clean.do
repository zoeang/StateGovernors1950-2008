...
gen election_code = 1000*year+statecode*10+party_id 
keep if incumbent == 1
drop if party == "INDEPENDENT"
drop if termlimit == "1 term limit"

save election2.dta, replace


*tabstat vote_share democratic_party, columns(statistics) statistics(n mean) by(pos)
